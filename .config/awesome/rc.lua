...
theme_path ="/home/pablo/.config/awesome/themes/default/theme"
...
apptags =
{
    ["Navigator"] = { screen = 1, tag = 2 },
		...
		["movil"] = {screen = 1, tag = 1},
    -- ["mocp"] = { screen = 2, tag = 4 },
		["Gimp"] = {screen = 1, tag = 6},
		-- envia el firefox download manager al escritorio 9
		["Download"] = { screen = 1, tag = 9},

		["lastfm"]={ screen =1, tag =8}
}
...
-- Mover client
keybinding({ modkey, "Control" }, "Up", function () awful.client.moveresize(0, -5, 0, 0) end):add()
keybinding({ modkey, "Control" }, "Down", function () awful.client.moveresize(0, 5, 0, 0) end):add()
...
keybinding({ modkey, "Mod1" }, "Right", function () awful.client.moveresize(0, 0, 5, 0) end):add()
...
awful.hooks.timer.register(20, function ()
    ...
    mytextbox.text = " " .. io.popen("/home/pablo/bin/showtemp.sh"):read() .. " " .. io.popen("/home/pablo/bin/showbat.sh"):read() .. " " .. os.date("%d") .. " " .. os.date("%H") .. ":" .. os.date("%M") .. " "
end)
-- }}}
require("funciones")
-- al inicio
os.execute("firefox &")
os.execute("xterm -g 144x20 -name movil -e screen &")
