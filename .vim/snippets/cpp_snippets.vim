...
" Author:       JPA
" Version:      0.1
" Last Change:  Fri Mar  6 09:20:14 CLST 2009
" ==========================================================
" cca:c:arglist {{{2
...
function! s:define_snippets()
    ...
    DefineSnippet for           for (<{int }><{i}> = <{0}>; <{i}> <{<}> <{len}>;<{i}><{<:@z=~'<'?'++':'--'}>){<CR><TAB><{}><CR><BS>}
...
if exists('loaded_cca')
    filetype indent on
    let b:{cca_filetype_ext_var} = 'cpp'
...
if exists('loaded_ctk')
    let b:{ctk_filetype_ext_var} = 'cpp'
